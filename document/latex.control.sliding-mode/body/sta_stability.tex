...
l
本节内容改编自\cite{morenoStrictLyapunovFunctions2014}，这是我目前最能看懂的证明，原文按照由简单到复杂的思路行文，我这里直接讨论非homogeneous的情况，希望可以给学习STA的朋友移除这个大山。
...
\begin{proposition}
	考虑STA系统
	\begin{equation}
		\dot{s}=-\sig{s}^{1/2}+w\quad 
		\dot{w}=-\sgn{s}+\delta(t),\quad 
		|\delta(t)|<\Delta
	\end{equation}
	其中， 增益的选择为$k_1>\frac{\sqrt{2}\nu^{1/2}}{\sqrt{k_2-\Delta}}$, $k_2>\Delta$, 其中$\nu>1$.
\end{proposition}

\begin{proof}
	定义一个微分同胚
	\begin{equation}
		\chi^T=[\chi_1,\chi_2]\quad
		\chi_1=\sig{s}^{1/2}  \quad
		\chi_2=\sig{s}^{1/2}-w
	\end{equation}
	于是，转换后的微分方程可以写为：
	\begin{equation}
