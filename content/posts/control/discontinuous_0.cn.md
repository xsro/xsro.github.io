+++
title = "非连续动态系统分析0——从经典滑模控制趋近律收敛性学起"
date = 2025-12-25
math = true
tags = ["控制理论", "非光滑"]
...
- 📌 这**不是逐点收敛**，而是**弱（平均）收敛**，或者说再等效控制（equivalent control）意义下收敛。
...
在滑模中，虽然有有限时间内 $s\to 0$的结论，但是$\dot{s}\to 0$是否收敛到零是一个棘手的课题。
在经典解的框架下，没有能够满足 $s\equiv 0$ 的解。
所以会引入Filippov意义下的解来使得 $s\equiv 0$ 满足原来的微分方程对应的微分包含。
但是，Filippov的凸化会导致存在多个解满足原来的微分方程，又难以刻画$\dot{s}\to 0$的结论。
因此所需的条件不连续项并非逐点收敛到扰动；其**平均效应**抵消了扰动，这一过程由Filippov凸化方法描述。
...
在非线性系统中，**等效控制方法**通过构造一个特定的控制律，从**Filippov解集**中选择一个稳定的轨迹，从而在某些情况下确保系统稳定性。这一过程的核心在于：  
1. **Filippov解集**（由不连续系统动态定义的解集合）可能包含多个轨迹，其中部分轨迹可能不稳定或非唯一。例如，在滑模面附近，Filippov解可能因不连续性导致多个可能的运动方向，而等效控制方法通过求解滑模面的条件（如 $\dot{s} = 0$）选择一个**稳定且唯一的轨迹**。  
2. **等效控制方法**通过构造等效控制 $u_{\text{eq}}$，使得系统在滑模面上运动，从而确保滑模动态的鲁棒性和稳定性。这一控制律的设计直接依赖于系统状态和扰动的补偿，使得即使在存在未建模扰动或参数不确定性的情况下，系统仍能保持稳定。  
3. **非线性系统的复杂性**可能导致Filippov解集的不唯一性，而等效控制方法通过引入连续化处理（如边界层方法或连续控制律）细化解集，选择一个满足稳定条件的轨迹。例如，在Utkin的著作中提到，对于控制线性系统，等效控制方法与Filippov解的结果一致，但在非线性系统中，等效控制方法通过构造滑模面方程（如 $\dot{x} = f - B(GB)^{-1}FG$）确保唯一性与稳定性。  

**结论**：等效控制方法通过选择一个稳定的滑模轨迹，从Filippov解集中提取出唯一且稳定的解，从而在非线性系统中实现鲁棒控制。尽管Filippov解集可能包含不稳定轨迹，但等效控制方法通过滑模设计确保了系统在滑模面的稳定性，因此在实际应用中，等效控制解是更可靠的选择 [1]。  
